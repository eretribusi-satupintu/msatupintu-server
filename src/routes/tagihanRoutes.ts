...
router.get(
  "/wajib-retribusi/:wr_id/sub-wilayah/:subwilayah_id",
  async (req: Request, res: Response) => {
    try {
      const data = await getTagihanWajibRetribusi(
        Number(req.params.wr_id),
...
router.get("/wajib-retribusi/:wr_id", async (req: Request, res: Response) => {
  try {
    const data = await getTagihanWajibRetribusiMasyarakat(
      Number(req.params.wr_id)
    );
    res.status(200).json({ data: data });
  } catch (error) {
...
router.get(
  "/wajib-retribusi/:wr_id/kontrak/:kontrak_id/progress",
  async (req: Request, res: Response) => {
    try {
      const data = await getTagihanWajibRetribusiMasyarakatProgress(
        Number(req.params.wr_id),
        Number(req.params.kontrak_id)
      );
      res.status(200).json({ data: data });
    } catch (error) {
      console.log(error);
...
router.get(
  "/petugas/:petugas_id/sub-wilayah/:subwilayah_id/status/:status",
  async (req: Request, res: Response) => {
    try {
